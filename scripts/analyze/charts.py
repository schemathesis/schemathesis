...
from .metrics import CallBucket, RunMetrics
...
def _bucket_chart(run: RunMetrics, charts_dir: Path) -> None:
    buckets_dict = dataclasses.asdict(run.buckets)
    fields = [bucket.value for bucket in CallBucket]
    counts = [buckets_dict[name] for name in fields]
    fig, ax = pyplot.subplots(figsize=(9, 4))
    seaborn.barplot(x=fields, y=counts, ax=ax, color="#3b82f6")
...
def _new_op_timeline_chart(run: RunMetrics, charts_dir: Path) -> None:
    ...
    ax.set_ylabel("Distinct ops with >= 1 2xx")
    _save(fig, charts_dir / "new_op_timeline")
