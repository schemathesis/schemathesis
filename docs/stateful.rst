...
Each event represents a phase in the state machine lifecycle, such as "before a step is executed", "after a step is executed", and others.
A test run contains of multiple test suites, with multiple test scenarios in each suite.
Schemathesis keep running test scenarios until the first test scenario that does not find any new failures.

Each test scenario is a sequence of random steps that are generated by the state machine.

.. important::

    Each test scenario that may include multiple API calls is considered as a single test case by Hypothesis.
    Therefore, the ``max_examples`` setting controls the number of test scenarios, not the number of API calls.

The available events are:

- ``BeforeRun`` - before the whole stateful test run.
- ``AfterRun`` - after the whole stateful test run.
- ``BeforeSuite`` - before every test suite.
- ``AfterSuite`` - after every test suite.
- ``BeforeScenario`` - before every test scenario.
- ``AfterScenario`` - after every test scenario.
- ``BeforeStep`` - before each step in a test scenario is executed.
- ``AfterStep`` - after each step in a test scenario is executed.
- ``Interrupted`` - when the test run is interrupted via Ctrl+C.
- ``Error`` - when an error occurs during the test run.

These events can be used to display the progress of the state machine test runner in the CLI, but they don't provide any control over the test execution.
