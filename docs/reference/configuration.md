...
!!! note ""
    ...
    **Default**: `"auto"`  
    ...
    Dependency-based ordering ensures operations that create resources run before operations that read them, which icreases the chances of finding bugs.
    ...
    Only available for non-stateful phases (examples, coverage, fuzzing).
