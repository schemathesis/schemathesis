...
# `3.38.8 <v3.38.7...v3.38.8>`{.interpreted-text role="version"} - 2024-11-28 {#v3.38.8}
...
-   Compatibility with Hypothesis \> 6.115.6. `2565`{.interpreted-text
    role="issue"}
...
# `3.34.1 <v3.34.0...v3.34.1>`{.interpreted-text role="version"} - 2024-08-20 {#v3.34.1}
...
# `3.32.2 <v3.32.1...v3.32.2>`{.interpreted-text role="version"} - 2024-07-17 {#v3.32.2}
...
# `3.27.0 <v3.26.2...v3.27.0>`{.interpreted-text role="version"} - 2024-04-14 {#v3.27.0}
...
# `3.24.3 <v3.24.2...v3.24.3>`{.interpreted-text role="version"} - 2024-01-23 {#v3.24.3}
...
-   Try to validate JSON data even if there is no proper `Content-Type`
    header. `1787`{.interpreted-text role="issue"}
...
-   Fixed type hint for `AuthProvider`. `1776`{.interpreted-text
    role="issue"}
...
# `3.18.5 <v3.18.4...v3.18.5>`{.interpreted-text role="version"} - 2023-02-18 {#v3.18.5}
...
# `3.16.3 <v3.16.2...v3.16.3>`{.interpreted-text role="version"} - 2022-08-08 {#v3.16.3}
...
# `3.10.1 <v3.10.0...v3.10.1>`{.interpreted-text role="version"} - 2021-10-04 {#v3.10.1}
...
# `3.7.4 <v3.7.3...v3.7.4>`{.interpreted-text role="version"} - 2021-05-28 {#v3.7.4}
...
# `3.7.0 <v3.6.11...v3.7.0>`{.interpreted-text role="version"} - 2021-05-23 {#v3.7.0}
...
-   Relax dependency on `starlette` to `>=0.13,<1`.
...
# `3.6.2 <v3.6.1...v3.6.2>`{.interpreted-text role="version"} - 2021-04-15 {#v3.6.2}
...
# `3.2.1 <v3.2.0...v3.2.1>`{.interpreted-text role="version"} - 2021-03-10 {#v3.2.1}
...
-   Loosen `importlib-metadata` version constraint and update
...
# `3.0.6 <v3.0.5...v3.0.6>`{.interpreted-text role="version"} - 2021-01-31 {#v3.0.6}

-   Packaging-only release for Docker images based on Debian Buster.
    `1028`{.interpreted-text role="issue"}

# `3.0.5 <v3.0.4...v3.0.5>`{.interpreted-text role="version"} - 2021-01-30 {#v3.0.5}
...
# `3.0.4 <v3.0.3...v3.0.4>`{.interpreted-text role="version"} - 2021-01-19 {#v3.0.4}

**Fixed**

-   Generating stateful tests, with common parameters behind a
    reference. `1020`{.interpreted-text role="issue"}
-   Programmatic addition of Open API links via `add_link` when schema
    ...
    integers. `1022`{.interpreted-text role="issue"}
...
-   When operations are resolved by `operationId` then the same
    ...
    deeply nested schemas. `945`{.interpreted-text role="issue"}

# `3.0.3 <v3.0.2...v3.0.3>`{.interpreted-text role="version"} - 2021-01-18 {#v3.0.3}

**Fixed**

-   `Flaky` Hypothesis error during explicit examples generation.
    `1018`{.interpreted-text role="issue"}

# `3.0.2 <v3.0.1...v3.0.2>`{.interpreted-text role="version"} - 2021-01-15 {#v3.0.2}

**Fixed**

-   Processing parameters common for multiple API operations if they are
    behind a reference. `1015`{.interpreted-text role="issue"}

# `3.0.1 <v3.0.0...v3.0.1>`{.interpreted-text role="version"} - 2021-01-15 {#v3.0.1}

**Added**

-   YAML serialization for `text/yaml`, `text/x-yaml`,
    `application/x-yaml` and `text/vnd.yaml` media types.
    `1010`{.interpreted-text role="issue"}.

# `3.0.0 <v2.8.6...v3.0.0>`{.interpreted-text role="version"} - 2021-01-14 {#v3.0.0}

**Added**

-   Support for sending `text/plain` payload as test data. Including
    variants with non-default `charset`. `850`{.interpreted-text
    role="issue"}, `939`{.interpreted-text role="issue"}
-   Generating data for all media types defined for an operation.
    `690`{.interpreted-text role="issue"}
...
-   The [response_schema_conformance]{.title-ref} check now runs on
    media types that are encoded with JSON. For example,
    `application/problem+json`. `920`{.interpreted-text role="issue"}
-   Base URL for GraphQL schemas. It allows you to load the schema from
    one place but send test requests to another one.
    `934`{.interpreted-text role="issue"}
-   A helpful error message when an operation is not found during the
    direct schema access. `812`{.interpreted-text role="issue"}
-   `--dry-run` CLI option. When applied, Schemathesis won\'t send any
    data to the server and won\'t perform any response checks.
    `963`{.interpreted-text role="issue"}
-   A better error message when the API schema contains an invalid
    regular expression syntax. `1003`{.interpreted-text role="issue"}
...
-   Missing `required: true` in path parameters definition is now
    ...
    (which is not compliant with the spec). `941`{.interpreted-text
    role="issue"}
-   Using `--auth` together with `--header` that sets the
    ...
    passed in `--auth` was used. `911`{.interpreted-text role="issue"}
...
-   If multiple non-check related failures happens during a test of a
    ...
    flaky tests. `975`{.interpreted-text role="issue"}
...
-   Display `Collected API operations` instead of `collected endpoints`
    in the CLI. `869`{.interpreted-text role="issue"}
-   `--skip-deprecated-endpoints` is renamed to
    `--skip-deprecated-operations`. `869`{.interpreted-text
    role="issue"}
-   Rename various internal API methods that contained `endpoint` in
    their names. `869`{.interpreted-text role="issue"}
...
-   Schemathesis doesn\'t stop testing on errors during schema parsing.
    ...
    failing the whole run. `999`{.interpreted-text role="issue"}

**Fixed**

-   Allow generating requests without payload if the schema does not
    require it. `916`{.interpreted-text role="issue"}
-   Allow sending `null` as request payload if the schema expects it.
    `919`{.interpreted-text role="issue"}
-   CLI failure if the tested operation is [GET]{.title-ref} and has
    payload examples. `925`{.interpreted-text role="issue"}
-   Excessive reference inlining that leads to out-of-memory for large
    schemas with deep references. `945`{.interpreted-text role="issue"},
    `671`{.interpreted-text role="issue"}
-   `--exitfirst` CLI option trims the progress bar output when a
    failure occurs. `951`{.interpreted-text role="issue"}
-   Internal error if filling missing explicit examples led to
    `Unsatisfiable` errors. `904`{.interpreted-text role="issue"}
-   Do not suggest to disable schema validation if it is already
    disabled. `914`{.interpreted-text role="issue"}
-   Skip explicit examples generation if this phase is disabled via
    config. `905`{.interpreted-text role="issue"}
-   `Unsatisfiable` error in stateful testing caused by all API
    operations having inbound links. `965`{.interpreted-text
    role="issue"}, `822`{.interpreted-text role="issue"}
-   A possibility to override `APIStateMachine.step`.
    `970`{.interpreted-text role="issue"}
-   `TypeError` on nullable parameters during Open API specific
    serialization. `980`{.interpreted-text role="issue"}
-   Invalid types in `x-examples`. `982`{.interpreted-text role="issue"}
-   CLI crash on schemas with operation names longer than the current
    terminal width. `990`{.interpreted-text role="issue"}
-   Handling of API operations that contain reserved characters in their
    paths. `992`{.interpreted-text role="issue"}
-   CLI execution stops on errors during example generation.
    `994`{.interpreted-text role="issue"}
-   Fill missing properties in incomplete explicit examples for non-body
    parameters. `1007`{.interpreted-text role="issue"}
...
-   Decrease CLI memory usage. `987`{.interpreted-text role="issue"}
...
This release features multiple backward-incompatible changes. The first
one is removing `form_data` and hooks related to it -all payload related
actions can be done via `body` and its hooks. The second one involves
renaming the so-called \"endpoint\" to \"operation\". The main reason
for this is to generalize terminology and make it applicable to GraphQL
schemas, as all Schemathesis internals are more suited to work with
semantically different API operations rather than with endpoints that
are often connected with URLs and HTTP methods. It brings the
possibility to reuse the same concepts for Open API and GraphQL - in the
future, unit tests will cover individual API operations in GraphQL,
rather than everything available under the same \"endpoint\".
:::

# `2.8.6 <v2.8.5...v2.8.6>`{.interpreted-text role="version"} - 2022-03-29 {#v2.8.6}

**Added**

-   Support for Werkzeug\>=2.1.0. `1410`{.interpreted-text role="issue"}

# `2.8.5 <v2.8.4...v2.8.5>`{.interpreted-text role="version"} - 2020-12-15 {#v2.8.5}

**Added**

-   `auto` variant for the `--workers` CLI option that automatically
    detects the number of available CPU cores to run tests on.
    `917`{.interpreted-text role="issue"}

# `2.8.4 <v2.8.3...v2.8.4>`{.interpreted-text role="version"} - 2020-11-27 {#v2.8.4}

**Fixed**

-   Use `--request-tls-verify` during schema loading as well.
    `897`{.interpreted-text role="issue"}

# `2.8.3 <v2.8.2...v2.8.3>`{.interpreted-text role="version"} - 2020-11-27 {#v2.8.3}

**Added**

-   Display failed response payload in the error output for the `pytest`
    plugin. `895`{.interpreted-text role="issue"}

**Changed**

-   In pytest plugin output, Schemathesis error classes use the
    [CheckFailed]{.title-ref} name. Before, they had not readable
    \"internal\" names.
-   Hypothesis falsifying examples. The code does not include `Case`
    attributes with default values to improve readability.
    `886`{.interpreted-text role="issue"}

# `2.8.2 <v2.8.1...v2.8.2>`{.interpreted-text role="version"} - 2020-11-25 {#v2.8.2}

**Fixed**

-   Internal error in CLI, when the `base_url` is an invalid IPv6.
    `890`{.interpreted-text role="issue"}
-   Internal error in CLI, when a malformed regex is passed to `-E` /
    `-M` / `-T` / `-O` CLI options. `889`{.interpreted-text
    role="issue"}

# `2.8.1 <v2.8.0...v2.8.1>`{.interpreted-text role="version"} - 2020-11-24 {#v2.8.1}

**Added**

-   `--force-schema-version` CLI option to force Schemathesis to use the
    specific Open API spec version when parsing the schema.
    `876`{.interpreted-text role="issue"}

**Changed**

-   The `content_type_conformance` check now raises a well-formed error
    message when encounters a malformed media type value.
    `877`{.interpreted-text role="issue"}

**Fixed**

-   Internal error during verifying explicit examples if an example has
    no `value` key. `882`{.interpreted-text role="issue"}

# `2.8.0 <v2.7.7...v2.8.0>`{.interpreted-text role="version"} - 2020-11-24 {#v2.8.0}

**Added**

-   `--request-tls-verify` CLI option, that controls whether
    Schemathesis verifies the server\'s TLS certificate. You can also
    pass the path to a CA_BUNDLE file for private certs.
    `830`{.interpreted-text role="issue"}

**Changed**

-   In CLI, if an endpoint contains an invalid schema, show a message
    about the `--validate-schema` CLI option. `855`{.interpreted-text
    role="issue"}
...
-   Handling of 204 responses in the `response_schema_conformance`
    ...
    `Content-Type` header. `844`{.interpreted-text role="issue"}
-   Catch `OverflowError` when an invalid regex is passed to `-E` / `-M`
    / `-T` / `-O` CLI options. `870`{.interpreted-text role="issue"}
-   Internal error in CLI, when the schema location is an invalid IPv6.
    `872`{.interpreted-text role="issue"}
-   Collecting Open API links behind references via CLI.
    `874`{.interpreted-text role="issue"}

**Deprecated**

-   Using of `Case.form_data` and `Endpoint.form_data`. In the `3.0`
    release, you\'ll need to use relevant `body` attributes instead.
    This change includes deprecation of the `before_generate_form_data`
    hook, use `before_generate_body` instead. The reason for this is the
...
# `2.7.7 <v2.7.6...v2.7.7>`{.interpreted-text role="version"} - 2020-11-13 {#v2.7.7}

**Fixed**

-   Missed `headers` in `Endpoint.partial_deepcopy`.

# `2.7.6 <v2.7.5...v2.7.6>`{.interpreted-text role="version"} - 2020-11-12 {#v2.7.6}
...
-   Pinned dependency on `attrs` that caused an error on fresh
    installations. `858`{.interpreted-text role="issue"}

# `2.7.5 <v2.7.4...v2.7.5>`{.interpreted-text role="version"} - 2020-11-09 {#v2.7.5}

**Fixed**

-   Invalid keyword in code samples that Schemathesis suggests to run to
    reproduce errors. `851`{.interpreted-text role="issue"}

# `2.7.4 <v2.7.3...v2.7.4>`{.interpreted-text role="version"} - 2020-11-07 {#v2.7.4}
...
# `2.7.3 <v2.7.2...v2.7.3>`{.interpreted-text role="version"} - 2020-11-05 {#v2.7.3}

**Fixed**

-   Internal error on malformed JSON when the `response_conformance`
    check is used. `832`{.interpreted-text role="issue"}

# `2.7.2 <v2.7.1...v2.7.2>`{.interpreted-text role="version"} - 2020-11-05 {#v2.7.2}

**Added**

-   Shortcut for response validation when Schemathesis\'s data
    generation is not used. `485`{.interpreted-text role="issue"}

**Changed**

-   Improve the error message when the application can not be loaded
    from the value passed to the `--app` command-line option.
...
-   Security definitions are now serialized as other parameters. At the
    ...
    Therefore this change is mostly for future-compatibility.
    `841`{.interpreted-text role="issue"}

**Fixed**

-   Internal error when a \"header\" / \"cookie\" parameter were not
    coerced to a string before filtration. `839`{.interpreted-text
    role="issue"}

# `2.7.1 <v2.7.0...v2.7.1>`{.interpreted-text role="version"} - 2020-10-22 {#v2.7.1}

**Fixed**

-   Adding new Open API links via the `add_link` method, when the
    related PathItem contains a reference. `824`{.interpreted-text
    role="issue"}

# `2.7.0 <v2.6.1...v2.7.0>`{.interpreted-text role="version"} - 2020-10-21 {#v2.7.0}

**Added**

-   New approach to stateful testing, based on the Hypothesis\'s
    `RuleBasedStateMachine`. `737`{.interpreted-text role="issue"}
...
-   The `response_schema_conformance` and `content_type_conformance`
    ...
    `Content-Type` header. `816`{.interpreted-text role="issue"}

**Fixed**

-   Failure reproduction code missing values that were explicitly passed
    to `call_*` methods during testing. `814`{.interpreted-text
    role="issue"}
...
# `2.6.1 <v2.6.0...v2.6.1>`{.interpreted-text role="version"} - 2020-10-19 {#v2.6.1}

**Added**

-   New method `as_curl_command` added to the `Case` class.
    `689`{.interpreted-text role="issue"}

# `2.6.0 <v2.5.1...v2.6.0>`{.interpreted-text role="version"} - 2020-10-06 {#v2.6.0}

**Added**

-   Support for passing Hypothesis strategies to tests created with
    `schema.parametrize` by using `schema.given` decorator.
    `768`{.interpreted-text role="issue"}
-   Support for PEP561. `748`{.interpreted-text role="issue"}
-   Shortcut for calling & validation. `738`{.interpreted-text
    role="issue"}
-   New hook to pre-commit, `rstcheck`, as well as updates to
    documentation based on rstcheck. `734`{.interpreted-text
    role="issue"}
-   New check for maximum response time and corresponding CLI option
    `--max-response-time`. `716`{.interpreted-text role="issue"}
-   New `response_headers_conformance` check that verifies the presence
    of all headers defined for a response. `742`{.interpreted-text
    role="issue"}
-   New field with information about executed checks in cassettes.
    `702`{.interpreted-text role="issue"}
-   New `port` parameter added to `from_uri()` method.
    `706`{.interpreted-text role="issue"}
-   A code snippet to reproduce a failed check when running Python
    tests. `793`{.interpreted-text role="issue"}
-   Python 3.9 support. `731`{.interpreted-text role="issue"}
-   Ability to skip deprecated endpoints with
    `--skip-deprecated-endpoints` CLI option and
    `skip_deprecated_operations=True` argument to schema loaders.
    `715`{.interpreted-text role="issue"}

**Fixed**

-   `User-Agent` header overriding the passed one.
    `757`{.interpreted-text role="issue"}
-   Default `User-Agent` header in `Case.call`. `717`{.interpreted-text
    role="issue"}
-   Status of individual interactions in VCR cassettes. Before this
    ...
    `695`{.interpreted-text role="issue"}
-   Escaping header values in VCR cassettes. `783`{.interpreted-text
    role="issue"}
-   Escaping HTTP response message in VCR cassettes.
    `788`{.interpreted-text role="issue"}
...
-   Speed up generation of `headers`, `cookies`, and `formData`
    parameters when their schemas do not define the `type` keyword.
    `795`{.interpreted-text role="issue"}

# `2.5.1 <v2.5.0...v2.5.1>`{.interpreted-text role="version"} - 2020-09-30 {#v2.5.1}

This release contains only documentation updates which are necessary to
upload to PyPI.

# `2.5.0 <v2.4.1...v2.5.0>`{.interpreted-text role="version"} - 2020-09-27 {#v2.5.0}

**Added**

-   Stateful testing via Open API links for the `pytest` runner.
    `616`{.interpreted-text role="issue"}
-   Support for GraphQL tests for the `pytest` runner.
    `649`{.interpreted-text role="issue"}

**Fixed**

-   Progress percentage in the terminal output for \"lazy\" schemas.
    `636`{.interpreted-text role="issue"}
...
# `2.4.1 <v2.4.0...v2.4.1>`{.interpreted-text role="version"} - 2020-09-17 {#v2.4.1}

**Changed**

-   Hide `Case.endpoint` from representation. Its representation
    decreases the usability of the pytest\'s output.
    `719`{.interpreted-text role="issue"}
-   Return registered functions from `register_target` and
    `register_check` decorators. `721`{.interpreted-text role="issue"}

**Fixed**

-   Possible `IndexError` when a user-defined check raises an exception
    without a message. `718`{.interpreted-text role="issue"}

# `2.4.0 <v2.3.4...v2.4.0>`{.interpreted-text role="version"} - 2020-09-15 {#v2.4.0}

**Added**

-   Ability to register custom targets for targeted testing.
    `686`{.interpreted-text role="issue"}
...
# `2.3.4 <v2.3.3...v2.3.4>`{.interpreted-text role="version"} - 2020-09-11 {#v2.3.4}

**Changed**

-   The default Hypothesis\'s `deadline` setting for tests with
    `schema.parametrize` is set to 500 ms for consistency with the CLI
    behavior. `705`{.interpreted-text role="issue"}

**Fixed**

-   Encoding error when writing a cassette on Windows.
    `708`{.interpreted-text role="issue"}

# `2.3.3 <v2.3.2...v2.3.3>`{.interpreted-text role="version"} - 2020-08-04 {#v2.3.3}

**Fixed**

-   `KeyError` during the `content_type_conformance` check if the
    response has no `Content-Type` header. `692`{.interpreted-text
    role="issue"}

# `2.3.2 <v2.3.1...v2.3.2>`{.interpreted-text role="version"} - 2020-08-04 {#v2.3.2}

**Added**

-   Run checks conditionally.

# `2.3.1 <v2.3.0...v2.3.1>`{.interpreted-text role="version"} - 2020-07-28 {#v2.3.1}

**Fixed**

-   `IndexError` when `examples` list is empty.

# `2.3.0 <v2.2.1...v2.3.0>`{.interpreted-text role="version"} - 2020-07-26 {#v2.3.0}

**Added**

-   Possibility to generate values for `in: formData` parameters that
    are non-bytes or contain non-bytes (e.g., inside an array).
    `665`{.interpreted-text role="issue"}
...
-   Error message for cases when a path parameter is in the template but
    ...
    its definition. `667`{.interpreted-text role="issue"}
-   Bump minimum required `hypothesis-jsonschema` version to
    [0.17.0]{.title-ref}. This allows Schemathesis to use the
    `custom_formats` argument in `from_schema` calls and avoid using its
    private API. `684`{.interpreted-text role="issue"}

**Fixed**

-   `ValueError` during sending a request with test payload if the
    endpoint defines a parameter with `type: array` and `in: formData`.
    `661`{.interpreted-text role="issue"}
-   `KeyError` while processing a schema with nullable parameters and
    `in: body`. `660`{.interpreted-text role="issue"}
-   `StopIteration` during `requestBody` processing if it has empty
    \"content\" value. `673`{.interpreted-text role="issue"}
-   `AttributeError` during generation of \"multipart/form-data\"
    parameters that have no \"type\" defined. `675`{.interpreted-text
    role="issue"}
-   Support for properties named \"\$ref\" in object schemas.
    Previously, it was causing `TypeError`. `672`{.interpreted-text
    role="issue"}
-   Generating illegal Unicode surrogates in the path.
    `668`{.interpreted-text role="issue"}
-   Invalid development dependency on `graphql-server-core` package.
    `658`{.interpreted-text role="issue"}

# `2.2.1 <v2.2.0...v2.2.1>`{.interpreted-text role="version"} - 2020-07-22 {#v2.2.1}

**Fixed**

-   Possible `UnicodeEncodeError` during generation of `Authorization`
    header values for endpoints with `basic` security scheme.
    `656`{.interpreted-text role="issue"}

# `2.2.0 <v2.1.0...v2.2.0>`{.interpreted-text role="version"} - 2020-07-14 {#v2.2.0}
...
-   Serialization of non-string parameters. `651`{.interpreted-text
    role="issue"}

# `2.1.0 <v2.0.0...v2.1.0>`{.interpreted-text role="version"} - 2020-07-06 {#v2.1.0}

**Added**

-   Support for property-level examples. `467`{.interpreted-text
    role="issue"}

**Fixed**

-   Content-type conformance check for cases when Open API 3.0 schemas
    contain \"default\" response definitions. `641`{.interpreted-text
    role="issue"}
-   Handling of multipart requests for Open API 3.0 schemas.
    `640`{.interpreted-text role="issue"}
-   Sending non-file form fields in multipart requests.
    `647`{.interpreted-text role="issue"}
...
# `2.0.0 <v1.10.0...v2.0.0>`{.interpreted-text role="version"} - 2020-07-01 {#v2.0.0}
...
the base path. `511`{.interpreted-text role="issue"}

**Fixed**

-   Show the correct URL in CLI progress when the base URL is
    overridden, including the path part. `511`{.interpreted-text
    role="issue"}
-   Construct valid URL when overriding base URL with base path.
    `511`{.interpreted-text role="issue"}
...
-   Support for hooks without [context]{.title-ref} argument in the
    first position.
...
# `1.10.0 <v1.9.1...v1.10.0>`{.interpreted-text role="version"} - 2020-06-28 {#v1.10.0}

**Added**

-   `loaders.from_asgi` supports making calls to ASGI-compliant
    application (For example: FastAPI). `521`{.interpreted-text
    role="issue"}
-   Support for GraphQL strategies.

**Fixed**

-   Passing custom headers to schema loader for WSGI / ASGI apps.
    `631`{.interpreted-text role="issue"}

# `1.9.1 <v1.9.0...v1.9.1>`{.interpreted-text role="version"} - 2020-06-21 {#v1.9.1}

**Fixed**

-   Schema validation error on schemas containing numeric values in
    scientific notation without a dot. `629`{.interpreted-text
    role="issue"}

# `1.9.0 <v1.8.0...v1.9.0>`{.interpreted-text role="version"} - 2020-06-20 {#v1.9.0}

**Added**

-   Pass the original case\'s response to the `add_case` hook.
-   Support for multiple examples with OpenAPI `examples`.
    `589`{.interpreted-text role="issue"}
-   `--verbosity` CLI option to minimize the error output.
    `598`{.interpreted-text role="issue"}
-   Allow registering function-level hooks without passing their name as
    the first argument to `apply`. `618`{.interpreted-text role="issue"}
-   Support for hook usage via `LazySchema` / `from_pytest_fixture`.
    `617`{.interpreted-text role="issue"}

**Changed**

-   Tests with invalid schemas marked as errors, instead of failures.
    `622`{.interpreted-text role="issue"}

**Fixed**

-   Crash during the generation of loosely-defined headers.
    `621`{.interpreted-text role="issue"}
-   Show exception information for test runs on invalid schemas with
    ...
    sections for invalid endpoints were empty. `622`{.interpreted-text
    role="issue"}

# `1.8.0 <v1.7.0...v1.8.0>`{.interpreted-text role="version"} - 2020-06-15 {#v1.8.0}

**Fixed**

-   Tests with invalid schemas are marked as failed instead of passed
    when `hypothesis-jsonschema>=0.16` is installed.
    `614`{.interpreted-text role="issue"}
-   `KeyError` during creating an endpoint strategy if it contains a
    reference. `612`{.interpreted-text role="issue"}

**Changed**

-   Require `hypothesis-jsonschema>=0.16`. `614`{.interpreted-text
    role="issue"}
-   Pass original `InvalidSchema` text to `pytest.fail` call.

# `1.7.0 <v1.6.3...v1.7.0>`{.interpreted-text role="version"} - 2020-05-30 {#v1.7.0}

**Added**

-   Support for YAML files in references via HTTPS & HTTP schemas.
    `600`{.interpreted-text role="issue"}
-   Stateful testing support via `Open API links` syntax.
    `548`{.interpreted-text role="issue"}
-   New `add_case` hook. `458`{.interpreted-text role="issue"}
-   Support for parameter serialization formats in Open API 2 / 3. For
    example `pipeDelimited` or `deepObject`. `599`{.interpreted-text
    role="issue"}
-   Support serializing parameters with `application/json` content-type.
    `594`{.interpreted-text role="issue"}
...
# `1.6.3 <v1.6.2...v1.6.3>`{.interpreted-text role="version"} - 2020-05-26 {#v1.6.3}

**Fixed**

-   Support for a colon symbol (`:`) inside of a header value passed
    via CLI. `596`{.interpreted-text role="issue"}

# `1.6.2 <v1.6.1...v1.6.2>`{.interpreted-text role="version"} - 2020-05-15 {#v1.6.2}

**Fixed**

-   Partially generated explicit examples are always valid and can be
    used in requests. `582`{.interpreted-text role="issue"}

# `1.6.1 <v1.6.0...v1.6.1>`{.interpreted-text role="version"} - 2020-05-13 {#v1.6.1}

**Changed**

-   Look at the current working directory when loading hooks for CLI.
    `586`{.interpreted-text role="issue"}

# `1.6.0 <v1.5.1...v1.6.0>`{.interpreted-text role="version"} - 2020-05-10 {#v1.6.0}

**Added**

-   New `before_add_examples` hook. `571`{.interpreted-text
    role="issue"}
-   New `after_init_cli_run_handlers` hook. `575`{.interpreted-text
    role="issue"}

**Fixed**

-   Passing `workers_num` to `ThreadPoolRunner` leads to always using 2
    workers in this worker kind. `579`{.interpreted-text role="issue"}

# `1.5.1 <v1.5.0...v1.5.1>`{.interpreted-text role="version"} - 2020-05-08 {#v1.5.1}

**Fixed**

-   Display proper headers in reproduction code when headers are
    overridden. `566`{.interpreted-text role="issue"}

# `1.5.0 <v1.4.0...v1.5.0>`{.interpreted-text role="version"} - 2020-05-06 {#v1.5.0}

**Added**

-   Display a suggestion to disable schema validation on schema loading
    errors in CLI. `531`{.interpreted-text role="issue"}
-   Filtration of endpoints by `operationId` via `operation_id`
    parameter to `schema.parametrize` or `-O` command-line option.
    `546`{.interpreted-text role="issue"}
-   Generation of security-related parameters. They are taken from
    ...
    parameters and generating data for HTTP authentication schemes.
    `540`{.interpreted-text role="issue"}

**Fixed**

-   Overriding header values in CLI and runner when headers provided
    explicitly clash with ones defined in the schema.
    `559`{.interpreted-text role="issue"}
-   Nested references resolving in `response_schema_conformance` check.
    `562`{.interpreted-text role="issue"}
-   Nullable parameters handling when they are behind a reference.
    `542`{.interpreted-text role="issue"}

# `1.4.0 <v1.3.4...v1.4.0>`{.interpreted-text role="version"} - 2020-05-03 {#v1.4.0}
...
-   Third-party compatibility fixups mechanism. Currently, there is one
    fixup for [FastAPI](https://github.com/tiangolo/fastapi).
    `503`{.interpreted-text role="issue"}
...
-   Reference resolving during response schema conformance check.
    `539`{.interpreted-text role="issue"}

# `1.3.4 <v1.3.3...v1.3.4>`{.interpreted-text role="version"} - 2020-04-30 {#v1.3.4}

**Fixed**

-   Validation of nullable properties in `response_schema_conformance`
    check introduced in `1.3.0`. `542`{.interpreted-text role="issue"}

# `1.3.3 <v1.3.2...v1.3.3>`{.interpreted-text role="version"} - 2020-04-29 {#v1.3.3}

**Changed**

-   Update `pytest-subtests` pin to `>=0.2.1,<1.0`.
    `537`{.interpreted-text role="issue"}

# `1.3.2 <v1.3.1...v1.3.2>`{.interpreted-text role="version"} - 2020-04-27 {#v1.3.2}
...
# `1.3.1 <v1.3.0...v1.3.1>`{.interpreted-text role="version"} - 2020-04-27 {#v1.3.1}

**Fixed**

-   Packaging issue

# `1.3.0 <v1.2.0...v1.3.0>`{.interpreted-text role="version"} - 2020-04-27 {#v1.3.0}
...
-   Storing network logs with `--store-network-log=<filename.yaml>`. The
    ...
    and contain extra information from the Schemathesis internals.
    `379`{.interpreted-text role="issue"}
-   Replaying of cassettes stored in VCR format. `519`{.interpreted-text
    role="issue"}
-   Targeted property-based testing in CLI and runner. It only supports
    the `response_time` target at the moment. `104`{.interpreted-text
    role="issue"}
-   Export CLI test results to JUnit.xml with
    `--junit-xml=<filename.xml>`. `427`{.interpreted-text role="issue"}

**Fixed**

-   Code samples for schemas where `body` is defined as
    `{"type": "string"}`. `521`{.interpreted-text role="issue"}
-   Showing error causes on internal `jsonschema` errors during input
    schema validation. `513`{.interpreted-text role="issue"}
-   Recursion error in `response_schema_conformance` check. Because of
    ...
    avoid recursion errors in `jsonschema` validation.
    `468`{.interpreted-text role="issue"}
...
-   Add a help message to the CLI output when an internal exception
    happens. `529`{.interpreted-text role="issue"}

# `1.2.0 <v1.1.2...v1.2.0>`{.interpreted-text role="version"} - 2020-04-15 {#v1.2.0}

**Added**

-   Per-test hooks for modification of data generation strategies.
    `492`{.interpreted-text role="issue"}
-   Support for `x-example` vendor extension in Open API 2.0.
    `504`{.interpreted-text role="issue"}
-   Sanity validation for the input schema & loader in `runner.prepare`.
    `499`{.interpreted-text role="issue"}

# `1.1.2 <v1.1.1...v1.1.2>`{.interpreted-text role="version"} - 2020-04-14 {#v1.1.2}

**Fixed**

-   Support for custom loaders in `runner`. Now all built-in loaders are
    supported as an argument to `runner.prepare`.
    `496`{.interpreted-text role="issue"}
-   `from_wsgi` loader accepts custom keyword arguments that will be
    passed to `client.get` when accessing the schema.
    `497`{.interpreted-text role="issue"}

# `1.1.1 <v1.1.0...v1.1.1>`{.interpreted-text role="version"} - 2020-04-12 {#v1.1.1}

**Fixed**

-   Mistakenly applied Open API -\> JSON Schema Draft 7 conversion. It
    should be Draft 4. `489`{.interpreted-text role="issue"}
-   Using wrong validator in `response_schema_conformance` check. It
    should be Draft 4 validator. `468`{.interpreted-text role="issue"}

# `1.1.0 <v1.0.5...v1.1.0>`{.interpreted-text role="version"} - 2020-04-08 {#v1.1.0}

**Fixed**

-   Response schema check for recursive schemas. `468`{.interpreted-text
    role="issue"}
...
# `1.0.5 <v1.0.4...v1.0.5>`{.interpreted-text role="version"} - 2020-04-03 {#v1.0.5}
...
-   Open API 3. Handling of endpoints that contain `multipart/form-data`
    ...
    correctly. `473`{.interpreted-text role="issue"}

# `1.0.4 <v1.0.3...v1.0.4>`{.interpreted-text role="version"} - 2020-04-03 {#v1.0.4}

**Fixed**

-   `OpenApi30.get_content_types` behavior, introduced in
    [8aeee1a](https://github.com/schemathesis/schemathesis/commit/8aeee1ab2c6c97d94272dde4790f5efac3951aed).
    `469`{.interpreted-text role="issue"}

# `1.0.3 <v1.0.2...v1.0.3>`{.interpreted-text role="version"} - 2020-04-03 {#v1.0.3}

**Fixed**

-   Precedence of `produces` keywords for Swagger 2.0 schemas. Now,
    operation-level `produces` overrides schema-level `produces` as
    specified in the specification. `463`{.interpreted-text
    role="issue"}
-   Content-type conformance check for Open API 3.0 schemas.
    `461`{.interpreted-text role="issue"}
-   Pytest 5.4 warning for test functions without parametrization.
    `451`{.interpreted-text role="issue"}

# `1.0.2 <v1.0.1...v1.0.2>`{.interpreted-text role="version"} - 2020-04-02 {#v1.0.2}

**Fixed**

-   Handling of fields in `paths` that are not operations, but allowed
    by the Open API spec. `457`{.interpreted-text role="issue"}
-   Pytest 5.4 warning about deprecated `Node` initialization usage.
    `451`{.interpreted-text role="issue"}

# `1.0.1 <v1.0.0...v1.0.1>`{.interpreted-text role="version"} - 2020-04-01 {#v1.0.1}

**Fixed**

-   Processing of explicit examples in Open API 3.0 when there are
    multiple parameters in the same location (e.g. `path`) contain
    `example` value. They are properly combined now.
    `450`{.interpreted-text role="issue"}

# `1.0.0 <v0.28.0...v1.0.0>`{.interpreted-text role="version"} - 2020-03-31 {#v1.0.0}
...
# `0.28.0 <v0.27.0...v0.28.0>`{.interpreted-text role="version"} - 2020-03-31 {#v0.28.0}

**Fixed**

-   Handling of schemas that use `x-*` custom properties.
    `448`{.interpreted-text role="issue"}

**Removed**

-   Deprecated `runner.execute`. Use `runner.prepare` instead.

# `0.27.0 <v0.26.1...v0.27.0>`{.interpreted-text role="version"} - 2020-03-31 {#v0.27.0}
...
# `0.26.1 <v0.26.0...v0.26.1>`{.interpreted-text role="version"} - 2020-03-24 {#v0.26.1}

**Fixed**

-   Limit recursion depth while resolving JSON schema to handle
    recursion without breaking. `435`{.interpreted-text role="issue"}

# `0.26.0 <v0.25.1...v0.26.0>`{.interpreted-text role="version"} - 2020-03-19 {#v0.26.0}

**Fixed**

-   Filter problematic path template variables containing `"/"`, or
    `"%2F"` url encoded. `440`{.interpreted-text role="issue"}
-   Filter invalid empty `""` path template variables.
    `439`{.interpreted-text role="issue"}
-   Typo in a help message in the CLI output. `436`{.interpreted-text
    role="issue"}

# `0.25.1 <v0.25.0...v0.25.1>`{.interpreted-text role="version"} - 2020-03-09 {#v0.25.1}

**Changed**

-   Allow `werkzeug` \>= 1.0.0. `433`{.interpreted-text role="issue"}

# `0.25.0 <v0.24.5...v0.25.0>`{.interpreted-text role="version"} - 2020-02-27 {#v0.25.0}
...
-   Handling of explicit examples from schemas. Now, if there are
    ...
    will be combined into a single example. `424`{.interpreted-text
    role="issue"}

# `0.24.5 <v0.24.4...v0.24.5>`{.interpreted-text role="version"} - 2020-02-26 {#v0.24.5}

**Fixed**

-   Error during `pytest` collection on objects with custom
    `__getattr__` method and therefore pass `is_schemathesis` check.
    `429`{.interpreted-text role="issue"}

# `0.24.4 <v0.24.3...v0.24.4>`{.interpreted-text role="version"} - 2020-02-22 {#v0.24.4}

**Fixed**

-   Resolving references when the schema is loaded from a file on
    Windows. `418`{.interpreted-text role="issue"}

# `0.24.3 <v0.24.2...v0.24.3>`{.interpreted-text role="version"} - 2020-02-10 {#v0.24.3}

**Fixed**

-   Not copied `validate_schema` parameter in `BaseSchema.parametrize`.
    Regression after implementing `383`{.interpreted-text role="issue"}
-   Missing `app`, `location` and `hooks` parameters in schema when used
    with `BaseSchema.parametrize`. `416`{.interpreted-text role="issue"}

# `0.24.2 <v0.24.1...v0.24.2>`{.interpreted-text role="version"} - 2020-02-09 {#v0.24.2}

**Fixed**

-   Crash on invalid regular expressions in `method`, `endpoint` and
    `tag` CLI options. `403`{.interpreted-text role="issue"}
-   Crash on a non-latin-1 encodable value in the `auth` CLI option.
    `404`{.interpreted-text role="issue"}
-   Crash on an invalid value in the `header` CLI option.
    `405`{.interpreted-text role="issue"}
-   Crash on some invalid URLs in the `schema` CLI option.
    `406`{.interpreted-text role="issue"}
-   Validation of `--request-timeout` parameter. `407`{.interpreted-text
    role="issue"}
-   Crash with `--hypothesis-deadline=0` CLI option.
    `410`{.interpreted-text role="issue"}
-   Crash with `--hypothesis-max-examples=0` CLI option.
    `412`{.interpreted-text role="issue"}

# `0.24.1 <v0.24.0...v0.24.1>`{.interpreted-text role="version"} - 2020-02-08 {#v0.24.1}

**Fixed**

-   CLI crash on Windows and Python \< 3.8 when the schema path contains
    characters unrepresentable at the OS level. `400`{.interpreted-text
    role="issue"}

# `0.24.0 <v0.23.7...v0.24.0>`{.interpreted-text role="version"} - 2020-02-07 {#v0.24.0}

**Added**

-   Support for testing of examples in Parameter & Media Type objects in
    Open API 3.0. `394`{.interpreted-text role="issue"}
-   `--show-error-tracebacks` CLI option to display errors\' tracebacks
    in the output. `391`{.interpreted-text role="issue"}
-   Support for schema behind auth. `115`{.interpreted-text
    role="issue"}
...
-   Schemas with GET endpoints accepting body are allowed now if schema
    ...
    with non-empty bodies. `383`{.interpreted-text role="issue"}

**Fixed**

-   CLI crash when an explicit example is specified in the endpoint
    definition. `386`{.interpreted-text role="issue"}

# `0.23.7 <v0.23.6...v0.23.7>`{.interpreted-text role="version"} - 2020-01-30 {#v0.23.7}

**Added**

-   `-x`/`--exitfirst` CLI option to exit after the first failed test.
    `378`{.interpreted-text role="issue"}

**Fixed**

-   Handling examples of parameters in Open API 3.0.
    `381`{.interpreted-text role="issue"}

# `0.23.6 <v0.23.5...v0.23.6>`{.interpreted-text role="version"} - 2020-01-28 {#v0.23.6}

**Added**

-   `all` variant for `--checks` CLI option to use all available checks.
    `374`{.interpreted-text role="issue"}

**Changed**

-   Use built-in `importlib.metadata` on Python 3.8.
    `376`{.interpreted-text role="issue"}

# `0.23.5 <v0.23.4...v0.23.5>`{.interpreted-text role="version"} - 2020-01-24 {#v0.23.5}

**Fixed**

-   Generation of invalid values in `Case.cookies`.
    `371`{.interpreted-text role="issue"}

# `0.23.4 <v0.23.3...v0.23.4>`{.interpreted-text role="version"} - 2020-01-22 {#v0.23.4}

**Fixed**

-   Converting `exclusiveMinimum` & `exclusiveMaximum` fields to JSON
    Schema. `367`{.interpreted-text role="issue"}

# `0.23.3 <v0.23.2...v0.23.3>`{.interpreted-text role="version"} - 2020-01-21 {#v0.23.3}

**Fixed**

-   Filter out surrogate pairs from the query string.

# `0.23.2 <v0.23.1...v0.23.2>`{.interpreted-text role="version"} - 2020-01-16 {#v0.23.2}

**Fixed**

-   Prevent `KeyError` when the response does not have the
    \"Content-Type\" header. `365`{.interpreted-text role="issue"}

# `0.23.1 <v0.23.0...v0.23.1>`{.interpreted-text role="version"} - 2020-01-15 {#v0.23.1}

**Fixed**

-   Dockerfile entrypoint was not working as per docs.
    `361`{.interpreted-text role="issue"}

# `0.23.0 <v0.22.0...v0.23.0>`{.interpreted-text role="version"} - 2020-01-15 {#v0.23.0}

**Added**

-   Hooks for strategy modification. `313`{.interpreted-text
    role="issue"}
-   Input schema validation. Use `--validate-schema=false` to disable it
    in CLI and `validate_schema=False` argument in loaders.
    `110`{.interpreted-text role="issue"}

# `0.22.0 <v0.21.0...v0.22.0>`{.interpreted-text role="version"} - 2020-01-11 {#v0.22.0}

**Added**

-   Show multiple found failures in the CLI output.
    `266`{.interpreted-text role="issue"} & `207`{.interpreted-text
    role="issue"}
-   Raise a proper exception when the given schema is invalid.
    `308`{.interpreted-text role="issue"}
-   Support for `None` as a value for `--hypothesis-deadline`.
    `349`{.interpreted-text role="issue"}

**Fixed**

-   Handling binary request payloads in `Case.call`.
    `350`{.interpreted-text role="issue"}
-   Type of the second argument to all built-in checks set to proper
    `Case` instead of `TestResult`. The error was didn\'t affect
    built-in checks since both `Case` and `TestResult` had `endpoint`
    attribute, and only it was used. However, this fix is not
    backward-compatible with 3rd party checks.

# `0.21.0 <v0.20.5...v0.21.0>`{.interpreted-text role="version"} - 2019-12-20 {#v0.21.0}

**Added**

-   Support for AioHTTP applications in CLI. `329`{.interpreted-text
    role="issue"}

# `0.20.5 <v0.20.4...v0.20.5>`{.interpreted-text role="version"} - 2019-12-18 {#v0.20.5}

**Fixed**

-   Compatibility with the latest release of `hypothesis-jsonschema` and
    setting its minimal required version to `0.9.13`.
    `338`{.interpreted-text role="issue"}

# `0.20.4 <v0.20.3...v0.20.4>`{.interpreted-text role="version"} - 2019-12-17 {#v0.20.4}

**Fixed**

-   Handling `nullable` attribute in Open API schemas.
    `335`{.interpreted-text role="issue"}

# `0.20.3 <v0.20.2...v0.20.3>`{.interpreted-text role="version"} - 2019-12-17 {#v0.20.3}

**Fixed**

-   Usage of the response status code conformance check with old
    `requests` version. `330`{.interpreted-text role="issue"}

# `0.20.2 <v0.20.1...v0.20.2>`{.interpreted-text role="version"} - 2019-12-14 {#v0.20.2}

**Fixed**

-   Response schema conformance check for Open API 3.0.
    `332`{.interpreted-text role="issue"}

# `0.20.1 <v0.20.0...v0.20.1>`{.interpreted-text role="version"} - 2019-12-13 {#v0.20.1}

**Added**

-   Support for response code ranges. `330`{.interpreted-text
    role="issue"}

# `0.20.0 <v0.19.1...v0.20.0>`{.interpreted-text role="version"} - 2019-12-12 {#v0.20.0}

**Added**

-   WSGI apps support. `31`{.interpreted-text role="issue"}
-   `Case.validate_response` for running built-in checks against app\'s
    response. `319`{.interpreted-text role="issue"}
...
# `0.19.1 <v0.19.0...v0.19.1>`{.interpreted-text role="version"} - 2019-12-11 {#v0.19.1}

**Fixed**

-   Compatibility with Hypothesis \>= 4.53.2. `322`{.interpreted-text
    role="issue"}

# `0.19.0 <v0.18.1...v0.19.0>`{.interpreted-text role="version"} - 2019-12-02 {#v0.19.0}

**Added**

-   Concurrent test execution in CLI / runner. `91`{.interpreted-text
    role="issue"}
-   update importlib_metadata pin to `^1.1`. `315`{.interpreted-text
    role="issue"}

# `0.18.1 <v0.18.0...v0.18.1>`{.interpreted-text role="version"} - 2019-11-28 {#v0.18.1}

**Fixed**

-   Validation of the `base-url` CLI parameter. `311`{.interpreted-text
    role="issue"}

# `0.18.0 <v0.17.0...v0.18.0>`{.interpreted-text role="version"} - 2019-11-27 {#v0.18.0}

**Added**

-   Resolving references in `PathItem` objects. `301`{.interpreted-text
    role="issue"}

**Fixed**

-   Resolving of relative paths in schemas. `303`{.interpreted-text
    role="issue"}
-   Loading string dates as `datetime.date` objects in YAML loader.
    `305`{.interpreted-text role="issue"}

# `0.17.0 <v0.16.0...v0.17.0>`{.interpreted-text role="version"} - 2019-11-21 {#v0.17.0}

**Added**

-   Resolving references that point to different files.
    `294`{.interpreted-text role="issue"}

**Changed**

-   Keyboard interrupt is now handled during the CLI run, and the
    summary is displayed in the output. `295`{.interpreted-text
    role="issue"}

# `0.16.0 <v0.15.0...v0.16.0>`{.interpreted-text role="version"} - 2019-11-19 {#v0.16.0}

**Added**

-   Display RNG seed in the CLI output to allow test reproducing.
    `267`{.interpreted-text role="issue"}
...
-   Refactor case generation strategies: strategy is not used to
    generate empty value. `253`{.interpreted-text role="issue"}
-   Improved error message for invalid path parameter declaration.
    `255`{.interpreted-text role="issue"}

**Fixed**

-   Pytest fixture parametrization via `pytest_generate_tests`.
    `280`{.interpreted-text role="issue"}
-   Support for tests defined as methods. `282`{.interpreted-text
    role="issue"}
-   Unclosed `requests.Session` on calling `Case.call` without passing a
    session explicitly. `286`{.interpreted-text role="issue"}

# `0.15.0 <v0.14.0...v0.15.0>`{.interpreted-text role="version"} - 2019-11-15 {#v0.15.0}

**Added**

-   Support for OpenAPI 3.0 server variables (base_path).
    `40`{.interpreted-text role="issue"}
-   Support for `format: byte`. `254`{.interpreted-text role="issue"}
-   Response schema conformance check in CLI / Runner.
    `256`{.interpreted-text role="issue"}
-   Docker image for CLI. `268`{.interpreted-text role="issue"}
-   Pre-run hooks for CLI. `147`{.interpreted-text role="issue"}
-   A way to register custom checks for CLI via
    `schemathesis.register_check`. `270`{.interpreted-text role="issue"}

**Fixed**

-   Not encoded path parameters. `272`{.interpreted-text role="issue"}

**Changed**

-   Verbose messages are displayed in the CLI on failed checks.
    `261`{.interpreted-text role="issue"}

# `0.14.0 <v0.13.2...v0.14.0>`{.interpreted-text role="version"} - 2019-11-09 {#v0.14.0}

**Added**

-   CLI: Support file paths in the `schema` argument.
    `119`{.interpreted-text role="issue"}
-   Checks to verify response status & content type in CLI / Runner.
    `101`{.interpreted-text role="issue"}

**Fixed**

-   Custom base URL handling in CLI / Runner. `248`{.interpreted-text
    role="issue"}

**Changed**

-   Raise an error if the schema has a body for GET requests.
    `218`{.interpreted-text role="issue"}
-   Method names are case insensitive during direct schema access.
    `246`{.interpreted-text role="issue"}

# `0.13.2 <v0.13.1...v0.13.2>`{.interpreted-text role="version"} - 2019-11-05 {#v0.13.2}

**Fixed**

-   `IndexError` when Hypothesis found inconsistent test results during
    the test execution in the runner. `236`{.interpreted-text
    role="issue"}

# `0.13.1 <v0.13.0...v0.13.1>`{.interpreted-text role="version"} - 2019-11-05 {#v0.13.1}

**Added**

-   Support for binary format `197`{.interpreted-text role="issue"}

**Fixed**

-   Error that happens when there are no success checks in the statistic
    in CLI. `237`{.interpreted-text role="issue"}

# `0.13.0 <v0.12.2...v0.13.0>`{.interpreted-text role="version"} - 2019-11-03 {#v0.13.0}

**Added**

-   An option to configure request timeout for CLI / Runner.
    `204`{.interpreted-text role="issue"}
-   A help snippet to reproduce errors caught by Schemathesis.
    `206`{.interpreted-text role="issue"}
-   Total running time to the CLI output. `181`{.interpreted-text
    role="issue"}
-   Summary line in the CLI output with the number of passed / failed /
    errored endpoint tests. `209`{.interpreted-text role="issue"}
-   Extra information to the CLI output: schema address, spec version,
    and base URL. `188`{.interpreted-text role="issue"}

**Fixed**

-   Compatibility with Hypothesis 4.42.4+ . `212`{.interpreted-text
    role="issue"}
-   Display flaky errors only in the \"ERRORS\" section and improve CLI
    output. `215`{.interpreted-text role="issue"}
-   Handling `formData` parameters in `Case.call`.
    `196`{.interpreted-text role="issue"}
-   Handling cookies in `Case.call`. `211`{.interpreted-text
    role="issue"}

**Changed**

-   More readable falsifying examples output. `127`{.interpreted-text
    role="issue"}
-   Show exceptions in a separate section of the CLI output.
    `203`{.interpreted-text role="issue"}
-   Error message for cases when it is not possible to satisfy schema
    parameters. It should be more clear now. `216`{.interpreted-text
    role="issue"}
-   Do not stop on schema errors related to a single endpoint.
    `139`{.interpreted-text role="issue"}
-   Display a proper error message when the schema is not available in
    CLI / Runner. `214`{.interpreted-text role="issue"}

# `0.12.2 <v0.12.1...v0.12.2>`{.interpreted-text role="version"} - 2019-10-30 {#v0.12.2}

**Fixed**

-   Wrong handling of the `base_url` parameter in runner and `Case.call`
    if it has a trailing slash. `194`{.interpreted-text role="issue"}
    and `199`{.interpreted-text role="issue"}
-   Do not send any payload with GET requests. `200`{.interpreted-text
    role="issue"}

# `0.12.1 <v0.12.0...v0.12.1>`{.interpreted-text role="version"} - 2019-10-28 {#v0.12.1}

**Fixed**

-   Handling for errors other than `AssertionError` and
    `HypothesisException` in the runner. `189`{.interpreted-text
    role="issue"}
-   CLI failing on the case when there are tests, but no checks were
    performed. `191`{.interpreted-text role="issue"}
...
# `0.12.0 <v0.11.0...v0.12.0>`{.interpreted-text role="version"} - 2019-10-28 {#v0.12.0}

**Added**

-   Display progress during the CLI run. `125`{.interpreted-text
    role="issue"}

**Fixed**

-   Test server-generated wrong schema when the `endpoints` option is
    passed via CLI. `173`{.interpreted-text role="issue"}
-   Error message if the schema is not found in CLI.
    `172`{.interpreted-text role="issue"}

**Changed**

-   Continue running tests on hypothesis error. `137`{.interpreted-text
    role="issue"}

# `0.11.0 <v0.10.0...v0.11.0>`{.interpreted-text role="version"} - 2019-10-22 {#v0.11.0}

**Added**

-   LazySchema accepts filters. `149`{.interpreted-text role="issue"}
-   Ability to register strategies for custom string formats.
    `94`{.interpreted-text role="issue"}
-   Generator-based events in the `runner` module to improve control
    over the execution flow.
-   Filtration by tags. `134`{.interpreted-text role="issue"}
...
-   Base URL in schema instances could be reused when it is defined
    ...
    optional in such cases. `153`{.interpreted-text role="issue"}
-   Hypothesis deadline is set to 500ms by default.
    `138`{.interpreted-text role="issue"}
...
-   User-agent during schema loading. `144`{.interpreted-text
    role="issue"}
-   Generation of invalid values in `Case.headers`.
    `167`{.interpreted-text role="issue"}

**Removed**

-   Undocumented support for `file://` URI schema

# `0.10.0 <v0.9.0...v0.10.0>`{.interpreted-text role="version"} - 2019-10-14 {#v0.10.0}

**Added**

-   HTTP Digest Auth support. `106`{.interpreted-text role="issue"}
-   Support for Hypothesis settings in CLI & Runner.
    `107`{.interpreted-text role="issue"}
-   `Case.call` and `Case.as_requests_kwargs` convenience methods.
    `109`{.interpreted-text role="issue"}
-   Local development server. `126`{.interpreted-text role="issue"}
...
# `0.9.0 <v0.8.1...v0.9.0>`{.interpreted-text role="version"} - 2019-10-09 {#v0.9.0}

**Added**

-   Test executor collects results of execution. `29`{.interpreted-text
    role="issue"}
-   CLI option `--base-url` for specifying base URL of API.
    `118`{.interpreted-text role="issue"}
-   Support for coroutine-based tests. `121`{.interpreted-text
    role="issue"}
-   User Agent to network requests in CLI & runner.
    `130`{.interpreted-text role="issue"}
...
-   Suppressed hypothesis exception about using `example`
    non-interactively. `92`{.interpreted-text role="issue"}

# `0.8.1 <v0.8.0...v0.8.1>`{.interpreted-text role="version"} - 2019-10-04 {#v0.8.1}
...
# `0.8.0 <v0.7.3...v0.8.0>`{.interpreted-text role="version"} - 2019-10-04 {#v0.8.0}

**Added**

-   CLI tool invoked by the `schemathesis` command.
    `30`{.interpreted-text role="issue"}
-   New arguments `api_options`, `loader_options` and `loader` for test
    executor. `90`{.interpreted-text role="issue"}
-   A mapping interface for schemas & convenience methods for direct
    strategy access. `98`{.interpreted-text role="issue"}

**Fixed**

-   Runner stopping on the first falsifying example.
    `99`{.interpreted-text role="issue"}

# `0.7.3 <v0.7.2...v0.7.3>`{.interpreted-text role="version"} - 2019-09-30 {#v0.7.3}

**Fixed**

-   Filtration in lazy loaders.

# `0.7.2 <v0.7.1...v0.7.2>`{.interpreted-text role="version"} - 2019-09-30 {#v0.7.2}

**Added**

-   Support for type \"file\" for Swagger 2.0. `78`{.interpreted-text
    role="issue"}
-   Support for filtering in loaders. `75`{.interpreted-text
    role="issue"}

**Fixed**

-   Conflict for lazy schema filtering. `64`{.interpreted-text
    role="issue"}

# `0.7.1 <v0.7.0...v0.7.1>`{.interpreted-text role="version"} - 2019-09-27 {#v0.7.1}

**Added**

-   Support for `x-nullable` extension. `45`{.interpreted-text
    role="issue"}

# `0.7.0 <v0.6.0...v0.7.0>`{.interpreted-text role="version"} - 2019-09-26 {#v0.7.0}

**Added**

-   Support for the `cookie` parameter in OpenAPI 3.0 schemas.
    `21`{.interpreted-text role="issue"}
-   Support for the `formData` parameter in Swagger 2.0 schemas.
    `6`{.interpreted-text role="issue"}
-   Test executor. `28`{.interpreted-text role="issue"}

**Fixed**

-   Using `hypothesis.settings` decorator with test functions created
    from `from_pytest_fixture` loader. `69`{.interpreted-text
    role="issue"}

# `0.6.0 <v0.5.0...v0.6.0>`{.interpreted-text role="version"} - 2019-09-24 {#v0.6.0}

**Added**

-   Parametrizing tests from a pytest fixture via `pytest-subtests`.
    `58`{.interpreted-text role="issue"}
...
# `0.5.0 <v0.4.1...v0.5.0>`{.interpreted-text role="version"} - 2019-09-16 {#v0.5.0}

**Added**

-   Generating explicit examples from the schema. `17`{.interpreted-text
    role="issue"}
...
-   Body resolving during test collection. `55`{.interpreted-text
    role="issue"}

# `0.4.1 <v0.4.0...v0.4.1>`{.interpreted-text role="version"} - 2019-09-11 {#v0.4.1}
...
# `0.4.0 <v0.3.0...v0.4.0>`{.interpreted-text role="version"} - 2019-09-10 {#v0.4.0}

**Fixed**

-   Resolving all inner references in objects. `34`{.interpreted-text
    role="issue"}

**Changed**

-   `jsonschema.RefResolver` is now used for reference resolving.
    `35`{.interpreted-text role="issue"}

# `0.3.0 <v0.2.0...v0.3.0>`{.interpreted-text role="version"} - 2019-09-06 {#v0.3.0}

**Added**

-   `Parametrizer.from_uri` method to construct parametrizer instances
    from URIs. `24`{.interpreted-text role="issue"}
...
# `0.2.0 <v0.1.0...v0.2.0>`{.interpreted-text role="version"} - 2019-09-05 {#v0.2.0}

**Added**

-   Open API 3.0 support. `10`{.interpreted-text role="issue"}
-   \"header\" parameters. `7`{.interpreted-text role="issue"}

**Changed**

-   Handle errors during collection / executions as failures.
-   Use `re.search` for pattern matching in
    `filter_method`/`filter_endpoint` instead of `fnmatch`.
    `18`{.interpreted-text role="issue"}
-   `Case.body` contains properties from the target schema, without the
    extra level of nesting.

**Fixed**

-   `KeyError` on collection when \"basePath\" is absent.
    `16`{.interpreted-text role="issue"}

# 0.1.0 - 2019-06-28 {#v0.1.0}
