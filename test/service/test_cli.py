...
def test_server_timeout(cli, schema_url, service, mocker):
    ...
    assert lines[20] in ("Compressed report size: 1 KB", "Compressed report size: 2 KB")
    assert lines[21] == f"Uploading reports to {service.base_url} ..."
    # Then the output indicates timeout
    assert lines[22] == "Upload: TIMEOUT"
