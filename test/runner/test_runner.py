...
def test_interactions(openapi3_base_url, real_app_schema, workers):
    ...
    # failure
    interactions = list(stream.find(events.AfterExecution, status=Status.FAILURE).forest.interactions.values())
    ...
    # success
    interactions = list(stream.find(events.AfterExecution, status=Status.SUCCESS).forest.interactions.values())
    assert len(interactions) == 1
...
def test_asgi_interactions(fastapi_app):
    ...
    interaction = list(event.forest.interactions.values())[0]
...
def test_empty_response_interaction(real_app_schema):
    ...
    interactions = list(stream.find(events.AfterExecution).forest.interactions.values())
...
def test_empty_string_response_interaction(real_app_schema):
    ...
    stream = EventStream(real_app_schema).execute()
    interactions = stream.find(events.AfterExecution).forest.interactions.values()
    for interaction in interactions:  # There could be multiple calls
...
def test_unknown_response_code(real_app_schema):
    ...
    check = list(stream.find(events.AfterExecution).forest.checks.values())[0][0]
...
def test_response_conformance_invalid(real_app_schema):
    ...
    # Then there should be a failure
    assert Status.FAILURE in stream.finished.outcome_statistic
    check = list(stream.find(events.AfterExecution).forest.checks.values())[-1][-1]
...
def test_explicit_examples_from_response(ctx, openapi3_base_url):
    ...
    assert [case.value.path_parameters for case in stream.find(events.AfterExecution).forest.cases.values()] == [
        {"itemId": "456789"},
        {"itemId": "123456"},
    ]
...
def test_hypothesis_errors_propagation(ctx, openapi3_base_url):
    ...
    assert sum(len(checks) for checks in after.forest.checks.values()) == max_examples
    assert Status.FAILURE not in stream.finished.outcome_statistic
    stream.assert_no_errors()
...
def test_graphql(graphql_url):
    ...
    for event, expected in zip(stream.find_all(events.AfterExecution), ["Query.getBooks", "Query.getAuthors"]):
        assert event.forest.label == expected
        for case in event.forest.cases.values():
...
def test_stateful_auth(real_app_schema):
    ...
    interactions = list(stream.find(events.ScenarioFinished).forest.interactions.values())
...
def test_stateful_all_generation_modes(real_app_schema):
    ...
    cases = list(stream.find(events.ScenarioFinished).forest.cases.values())
    assert len(cases) > 0
    for case in cases:
...
def test_stateful_seed(real_app_schema):
    ...
    for _ in range(3):
        ...
        current = []
        interactions = stream.find(events.ScenarioFinished).forest.interactions
...
def test_stateful_override(real_app_schema):
    ...
    interactions = stream.find(events.ScenarioFinished).forest.interactions.values()
    assert len(interactions) > 0
