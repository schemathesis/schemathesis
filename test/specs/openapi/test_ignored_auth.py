...
def test_auth_is_not_checked(with_generated, schema_url):
    ...
    forest = event.forest
    case = list(forest.cases.values())[-1].value
    check = forest.checks[case.id][-1]
    assert check.status == Status.FAILURE
    assert check.name == "ignored_auth"
    headers = forest.cases[case.id].value.headers or {}
    response = forest.interactions[case.id].response
    if with_generated:
