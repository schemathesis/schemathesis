...
def test_propagation_writes_required_at_each_ancestor_inline(materialize_targets):
    schema = {
        "type": "object",
        "properties": {
    ...
    targets = materialize_targets(schema, descriptors)
...
def test_propagation_writes_required_through_bundled_refs(materialize_targets):
    schema = {
        "$ref": "#/x-bundled/outer",
        BUNDLE_STORAGE_KEY: {
            "outer": {
...
def test_propagation_is_idempotent(materialize_targets):
...
def test_propagation_collapses_oneof_to_chosen_branch(materialize_targets):
    schema = {
        "oneOf": [
            {"type": "string"},
...
def test_propagation_handles_array_items_min(materialize_targets):
...
def test_propagation_synthesizes_property_for_additional_properties(materialize_targets):
...
def test_propagation_synthesizes_unique_property_when_default_taken(materialize_targets):
...
def test_propagation_synthesizes_pattern_property_for_simple_pattern(materialize_targets):
...
def test_propagation_skips_pattern_property_when_no_matching_name_can_be_synthesized(materialize_targets):
...
def test_propagation_synthesizes_property_name_that_matches_pattern(pattern, materialize_targets):
...
def test_propagation_skips_when_no_candidate_satisfies_pattern(pattern, materialize_targets):
    pattern_schema = {"type": "string", "pattern": "^a$"}
    schema = {"type": "object", "patternProperties": {pattern: pattern_schema}}
    descriptors = compute_mutation_targets(schema)
    targets = materialize_targets(schema, descriptors)
...
def test_propagation_allof_branch_keeps_siblings(materialize_targets):
