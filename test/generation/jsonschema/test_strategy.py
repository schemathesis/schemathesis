    ...
    if properties:
        ...
        if draw(st.integers(0, 3)) == 0:
            ...
            schema["dependentSchemas"] = {
                trigger: {"required": draw(st.lists(names, unique=True, min_size=1, max_size=2))}
            }
    if patterns:
...
OBJECT_WITNESS_SCHEMAS = [
    ...
    },
    {
        "type": "object",
        "properties": {"a": {"type": "integer"}},
        "dependentSchemas": {"a": {"required": ["b"], "properties": {"b": {"type": "string"}}}},
    },
    {
        "type": "object",
...
@pytest.mark.parametrize(
    "schema",
    [
        # both patterns must hold, not only the first
        {"type": "string", "allOf": [{"pattern": "^a"}, {"pattern": "b$"}]},
        # a negated pattern must be excluded from generated values
        {"type": "string", "not": {"pattern": "^a"}},
    ],
    ids=str,
)
def test_string_secondary_and_negative_patterns_are_sound(schema):
    canonical = jsonschema_rs.canonicalize(schema, inline_budget=0)
    view = canonical.view()
    # Confirm the schema reaches the targeted StringView shape.
    assert isinstance(view, jsonschema_rs.canonical.StringView)
    assert len(view.patterns) > 1 or view.not_patterns, (
        f"schema did not produce the targeted StringView shape: "
        f"patterns={view.patterns} not_patterns={view.not_patterns}"
    )
