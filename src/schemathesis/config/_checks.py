...
class CheckConfig(DiffBase):
    ...
    def __init__(self, *, enabled: bool = False, expected_statuses: list[int] | None = None) -> None:
        self.enabled = enabled
...
class ChecksConfig(DiffBase):
    ...
    def from_dict(cls, data: dict[str, Any]) -> ChecksConfig:
        return cls(
            not_a_server_error=CheckConfig.from_dict(data.get("not_a_server_error", {})),
            status_code_conformance=SimpleCheckConfig.from_dict(data.get("status_code_conformance", {})),
            content_type_conformance=SimpleCheckConfig.from_dict(data.get("content_type_conformance", {})),
            response_schema_conformance=SimpleCheckConfig.from_dict(data.get("response_schema_conformance", {})),
            positive_data_acceptance=CheckConfig.from_dict(data.get("positive_data_acceptance", {})),
            negative_data_rejection=CheckConfig.from_dict(data.get("negative_data_rejection", {})),
            use_after_free=SimpleCheckConfig.from_dict(data.get("use_after_free", {})),
            ensure_resource_availability=SimpleCheckConfig.from_dict(data.get("ensure_resource_availability", {})),
            missing_required_header=CheckConfig.from_dict(data.get("missing_required_header", {})),
            ignored_auth=SimpleCheckConfig.from_dict(data.get("ignored_auth", {})),
        )
