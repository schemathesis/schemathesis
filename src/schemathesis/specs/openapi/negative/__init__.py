...
def negative_schema(
    ...
    if location in ("header", "cookie", "query"):
        ...
        mutations = (remove_required_property, negate_constraints, change_properties)
    elif location == "path":
    ...
    return mutated().flatmap(lambda s: from_schema({"allOf": [s, {"not": schema}]}, custom_formats=custom_formats))
