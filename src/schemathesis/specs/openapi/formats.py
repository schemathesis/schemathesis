...
_DOMAIN_LABEL_CHARACTERS = string.ascii_lowercase + string.digits
_TOP_LEVEL_DOMAINS = ("com", "org", "net", "io", "dev", "info", "example", "test")
_LOCAL_PART_CHARACTERS = string.ascii_lowercase + string.digits
...
def domain_values() -> st.SearchStrategy[str]:
    ...
    # Assembled from a fixed label alphabet rather than drawn from `hypothesis.provisional.domains`,
    # which costs ~1.3ms per value and emits IDN labels with a mixed-case `xn--` prefix that
    # validators reject per RFC 3490.
    label = st.text(alphabet=_DOMAIN_LABEL_CHARACTERS, min_size=1, max_size=8)
    return st.builds(
...
def color_values() -> st.SearchStrategy[str]:
    ...
    short = st.integers(0, 0xFFF).map("#{:03x}".format)
    long = st.integers(0, 0xFFFFFF).map("#{:06x}".format)
...
_REGEX_LITERAL_CHARACTERS = string.ascii_letters + string.digits + "_- :;,/@"
...
def regex_values() -> st.SearchStrategy[str]:
    ...
    atom = st.sampled_from(_REGEX_ATOMS) | st.text(alphabet=_REGEX_LITERAL_CHARACTERS, min_size=1, max_size=3)
...
def json_pointer_values(alphabet: st.SearchStrategy[str]) -> st.SearchStrategy[str]:
    ...
    # Bounded: an unbounded list of unbounded tokens costs a third more and says nothing extra.
