...
OPENAPI_32 = {
    ...
    "type": "object",
    "properties": {
        "openapi": {"type": "string", "pattern": "^3\\.2\\.\\d+(-.+)?$"},
        "$self": {
            "type": "string",
    ...
    "$defs": {
        ...
        "components": {
            ...
            "properties": {
                ...
                "callbacks": {"type": "object", "additionalProperties": {"$ref": "#/$defs/callbacks-or-reference"}},
                "pathItems": {"type": "object", "additionalProperties": {"$ref": "#/$defs/path-item"}},
                "mediaTypes": {"type": "object", "additionalProperties": {"$ref": "#/$defs/media-type-or-reference"}},
            },
            "patternProperties": {
        ...
        "parameters": {
            ...
            "not": {
                "allOf": [
                    {"contains": {"type": "object", "properties": {"in": {"const": "query"}}, "required": ["in"]}},
                    {
                        "contains": {
                            "type": "object",
                            "properties": {"in": {"const": "querystring"}},
                            "required": ["in"],
                        }
                    },
                ]
            },
            "contains": {"type": "object", "properties": {"in": {"const": "querystring"}}, "required": ["in"]},
            "minContains": 0,
            "maxContains": 1,
        },
        "parameter": {
            ...
            "properties": {
                "name": {"type": "string"},
                "in": {"enum": ["query", "querystring", "header", "path", "cookie"]},
                "description": {"type": "string"},
                "required": {"default": False, "type": "boolean"},
                "deprecated": {"default": False, "type": "boolean"},
                "schema": {"$dynamicRef": "#meta"},
            ...
            "allOf": [
                ...
                {"if": {"properties": {"in": {"const": "querystring"}}}, "then": {"required": ["content"]}},
            ],
            "dependentSchemas": {
                "schema": {
                    "properties": {
                    ...
                    "$defs": {
                        ...
                        "styles-for-cookie": {
                            ...
                            "then": {"properties": {"style": {"default": "form", "enum": ["form", "cookie"]}}},
                        },
                    },
                }
            },
        ...
        "content": {
            ...
            "type": "object",
            "additionalProperties": {"$ref": "#/$defs/media-type-or-reference"},
            "propertyNames": {"format": "media-range"},
        },
        "media-type": {
            ...
            "properties": {
                "description": {"type": "string"},
                "schema": {"$dynamicRef": "#meta"},
                "itemSchema": {"$dynamicRef": "#meta"},
                "encoding": {"type": "object", "additionalProperties": {"$ref": "#/$defs/encoding"}},
                "prefixEncoding": {"type": "array", "items": {"$ref": "#/$defs/encoding"}},
                "itemEncoding": {"$ref": "#/$defs/encoding"},
            },
            "dependentSchemas": {"encoding": {"properties": {"prefixEncoding": False, "itemEncoding": False}}},
            "allOf": [{"$ref": "#/$defs/examples"}, {"$ref": "#/$defs/specification-extensions"}],
            "unevaluatedProperties": False,
        },
        "media-type-or-reference": {
            "if": {"type": "object", "required": ["$ref"]},
            "then": {"$ref": "#/$defs/reference"},
            "else": {"$ref": "#/$defs/media-type"},
        },
        "encoding": {
            ...
            "dependentSchemas": {
                "encoding": {"properties": {"prefixEncoding": False, "itemEncoding": False}},
                "style": {"properties": {"allowReserved": {"default": False}}},
                "explode": {"properties": {"style": {"default": "form"}, "allowReserved": {"default": False}}},
        ...
        "example": {
            ...
            "allOf": [
                {"not": {"required": ["value", "externalValue"]}},
                {"not": {"required": ["value", "dataValue"]}},
                {"not": {"required": ["value", "serializedValue"]}},
                {"not": {"required": ["serializedValue", "externalValue"]}},
            ],
            "$ref": "#/$defs/specification-extensions",
            "unevaluatedProperties": False,
        },
                ...
                "type-oauth2": {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "oauth2MetadataUrl": {"type": "string", "format": "uri-reference"},
                        },
                        "required": ["flows"],
                    },
                },
        ...
        "oauth-flows": {
            ...
            "properties": {
                ...
                "clientCredentials": {"$ref": "#/$defs/oauth-flows/$defs/client-credentials"},
                "authorizationCode": {"$ref": "#/$defs/oauth-flows/$defs/authorization-code"},
                "deviceAuthorization": {"$ref": "#/$defs/oauth-flows/$defs/device-authorization"},
            },
            "$ref": "#/$defs/specification-extensions",
            "unevaluatedProperties": False,
            "$defs": {
                ...
                "device-authorization": {
                    "type": "object",
                    "properties": {
                        "deviceAuthorizationUrl": {"type": "string", "format": "uri-reference"},
                        "tokenUrl": {"type": "string", "format": "uri-reference"},
                        "refreshUrl": {"type": "string", "format": "uri-reference"},
                        "scopes": {"$ref": "#/$defs/map-of-strings"},
                    },
