...
def compute_dependency_layers(graph: DependencyGraph) -> list[list[str]] | None:
    ...
    for op_label, op_node in graph.operations.items():
        for output_slot in op_node.outputs:
            resource_id = id(output_slot.resource)
            producers[resource_id].add(op_label)

    # Build dependency edges
    for op_label, op_node in graph.operations.items():
        for input_slot in op_node.inputs:
            ...
            for producer_label in producers[resource_id]:
                ...
                if producer_label != op_label:
                    dependencies[op_label].add(producer_label)
    ...
    while queue:
        ...
        for _ in range(layer_size):
            op_label = queue.popleft()
            current_layer.append(op_label)
            processed.add(op_label)
    ...
    if len(processed) < len(graph.operations):
        ...
        # Add remaining operations to final layer, sorted for determinism
...
def _compute_layers_with_cycles(
    ...
    # Add any remaining SCCs (shouldn't happen if SCC algorithm is correct)
    if len(processed_sccs) < len(sccs):
        remaining: list[str] = []
        for scc_idx in range(len(sccs)):
            if scc_idx not in processed_sccs:
                remaining.extend(sccs[scc_idx])
        if remaining:
            remaining.sort()
            layers.append(remaining)
