...
def try_unwrap_all_of(schema: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    for alt_schema in alternatives:
        if _is_interesting_schema(alt_schema):
...
def unwrap_schema(schema: Mapping[str, Any], path: str, resolver: RefResolver) -> UnwrappedSchema:
    # Array at root
    ...
    # Pagination wrapper
    array_field = _is_pagination_wrapper(schema, resolver)
    if array_field:
...
def _is_pagination_wrapper(schema: Mapping[str, Any], resolver: RefResolver) -> str | None:
