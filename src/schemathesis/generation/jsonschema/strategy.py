...
def _integer(view: jsonschema_rs.canonical.IntegerView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    if lo > hi:
        ...
        return st.nothing()
    return st.integers(min_value=lo, max_value=hi).map(lambda k: k * multiple)
...
def _number(view: jsonschema_rs.canonical.NumberView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    if multiple is not None:
        ...
        if klo is not None and khi is not None and klo > khi:
            return st.nothing()
        return st.integers(min_value=klo, max_value=khi).map(lambda k: k * multiple)
...
def _string(view: jsonschema_rs.canonical.StringView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    pattern = view.patterns[0] if view.patterns else None
...
def _array(view: jsonschema_rs.canonical.ArrayView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    prefix = [from_schema(item, ctx) for item in view.prefix]
...
def _object(
    ...
    required = [r.name for r in view.requirements if isinstance(r, canon.RequiredReq)]
    dependent_required = [(r.trigger, list(r.required)) for r in view.requirements if isinstance(r, canon.DependentRequiredReq)]
    pattern_required = [r.pattern for r in view.requirements if isinstance(r, canon.PatternRequiredReq)]
    exact = {c.name: from_schema(c.value, ctx) for c in view.constraints if isinstance(c, canon.ExactConstraint)}
    patterns = [(c.pattern, from_schema(c.value, ctx)) for c in view.constraints if isinstance(c, canon.PatternConstraint)]
...
def _object_checks(
    ...
    for requirement in view.requirements:
        if isinstance(requirement, jsonschema_rs.canonical.DependentSchemaReq):
            try:
                is_valid = _accepts(requirement.schema.to_json_schema())
            except ValueError:
                continue
            dependent.append((requirement.trigger, is_valid))
    if dependent:
...
def _additional_value(
    ...
    for c in view.constraints:
        if isinstance(c, jsonschema_rs.canonical.AdditionalConstraint):
            if isinstance(c.value.view(), jsonschema_rs.canonical.FalseView):
                return None
            return from_schema(c.value, ctx)
    # Absent `additionalProperties` allows any extra property.
    return _anything(ctx)


def _property_name(
    ctx: StrategyContext, property_names: jsonschema_rs.CanonicalSchema | None
) -> SearchStrategy[str]:
...
def _all_of(view: jsonschema_rs.canonical.AllOfView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    if not isinstance(rebuilt.view(), jsonschema_rs.canonical.AllOfView):
        ...
        base = next((b for b in branches if not isinstance(b.view(), jsonschema_rs.canonical.NotView)), branches[0])
    return from_schema(base, ctx).filter(_accepts(merged.to_json_schema()))
