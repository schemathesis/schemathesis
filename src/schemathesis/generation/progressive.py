...
@dataclass
class Controller:
    """Sidecar control surface for a `CaseGenerator`.

    Exposes `stop()` to halt iteration externally and `deferred_errors`
    for pre-iteration failures the engine surfaces after iteration ends.
    """

    _stop: bool = field(default=False, init=False, repr=False)
    deferred_errors: list[Exception] = field(default_factory=list)
    invalid_headers: dict[str, str] | None = None
