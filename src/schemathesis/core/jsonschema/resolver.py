...
# Per-document fragment-walk cache; reset when a base_uri's root schema is replaced.
# `RefResolver` cached its analogous `_remote_cache`. Walking JSON pointers in pure
# Python is otherwise the bundler's hottest non-trivial cost.
...
def _resolve_reference_uri_with_document(base_uri: str, reference: str) -> tuple[str, str]:
    ...
    duplicate `$ref`. `RefResolver` (the previous implementation) cached urljoin too.
    """
    ...
    resolved = f"{resolved_path}#{fragment}" if fragment else resolved_path
    return resolved, document_uri
...
def _resolve_local_fragment(document_uri: str, fragment: str) -> Any:
    """Walk the cached root schema for `document_uri` to `fragment`. Returns UNRESOLVABLE on miss.

    Cached per-document because the bundler resolves the same fragment many times during a
    single bundle pass — `RefResolver` cached its analogous `_remote_cache` for the same reason.
    """
    ...
    if cached is not _FRAGMENT_MISS:
        return cached
    value = original if not fragment else resolve_pointer(original, fragment)
...
def resolve_reference_with_uri(
    ...
    """Resolve a `$ref` and also return the absolute URI of the target.

    Hot-path callers (the bundler walks every `$ref`) need both the URI — for
    cycle and visited-set bookkeeping — and the resolved value. Computing them
    together avoids a second pass through `resolve_reference_uri`.
    """
    try:
