...
def has_responses(recorder: ScenarioRecorder) -> bool:
    return any(interaction.response is not None for interaction in recorder.interactions.values())
...
def run_one_case(
    *,
    ...
    try:
        ...
        if isinstance(exc, requests.ConnectionError) and ctx.detect_server_outage(exc, workers=ctx.config.workers):
